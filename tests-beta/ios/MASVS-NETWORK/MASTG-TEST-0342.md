...
platform: ios
title: >-
  Info.plist 内の脆弱な ATS TLS ポリシー例外への参照 (References to Weak ATS TLS Policy
...
# MASTG-TEST-0342 Info.plist 内の脆弱な ATS TLS ポリシー例外への参照 (References to Weak ATS TLS Policy Exceptions in Info.plist)
...
3. [ATS 構成の解析 (Analyzing the ATS Configuration)](https://github.com/coky-t/owasp-mastg-ja/blob/master/techniques/ios/MASTG-TECH-0155.md) を使用して、TLS ポリシー例外、特に `NSExceptionMinimumTLSVersion`, `NSExceptionRequiresForwardSecrecy`, `NSAllowsArbitraryLoads`、について ATS 構成を解析します。
