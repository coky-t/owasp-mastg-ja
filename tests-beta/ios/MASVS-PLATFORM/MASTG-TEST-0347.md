...
アプリが機密データを含むテキスト入力フィールドをマスクしない場合、そのようなデータは傍観者 (ショルダーサーフィン) に見えたり、スクリーンショットやスクリーンレコーディングでキャプチャされる可能性があります。フィールドをセキュアにマークしても、そこはシステムキーボードを維持し続けます。iOS ではセキュアなフィールドに対してインストール済みサードパーティ (カスタム) キーボードを提供しない ([カスタムキーボード (Custom Keyboards)](../../../knowledge/ios/MASVS-PLATFORM/MASTG-KNOW-0141.md) を参照) ため、それらは入力した文字を受信することはありません。
