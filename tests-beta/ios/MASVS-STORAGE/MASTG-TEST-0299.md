...
これはアプリのプライベートストレージ (サンドボックス) だけでなく、アプリとその拡張機能が使用する [アプリグループ共有コンテナ](https://developer.apple.com/documentation/bundleresources/entitlements/com_apple_security_application-groups) にも適用されます ([App Extension (App extensions)](../../../knowledge/ios/MASVS-PLATFORM/MASTG-KNOW-0082.md) を参照)。共有コンテナ内のファイルにも、プライベートストレージと同様にデータ保護を適用するためです。
...
3. [アプリデータディレクトリのアクセス (Accessing App Data Directories)](../../../techniques/ios/MASTG-TECH-0059.md) を使用して、アプリのプライベートストレージ (サンドボックス) ディレクトリツリー (`/var/mobile/Containers/Data/Application/<UUID>/`) およびアプリグループ共有コンテナ (`/private/var/mobile/Containers/Shared/AppGroup/<UUID>/`) からデータ保護クラスを含むファイルのリストを取得します。
