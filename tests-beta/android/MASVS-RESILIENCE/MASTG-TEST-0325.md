...
アプリがこのテストで使用されるフックやトレースでカバーされていないルート検出技法を使用している場合、またはルート検出ロジックが検出を回避するような方法で (たとえば、難読化、動的コードローディング、アンチ計装技法などを通じて) 実装されている場合、このテストは検出漏れを生み出す可能性があります。そのような場合には、発見されないことがルート検出がないことを保証するものではなく、ルート検出メカニズムを特定および解析するには、追加の手動リバースエンジニアリングやカスタム計装が必要になることがあります。
