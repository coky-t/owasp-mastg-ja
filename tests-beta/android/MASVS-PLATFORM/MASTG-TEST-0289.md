...
# MASTG-TEST-0289 アプリのバックグラウンド時のスクリーンショットでの機密コンテンツ露出の実行時検証 (Runtime Verification of Sensitive Content Exposure in Screenshots During App Backgrounding)
...
2. [ホストとデバイス間のデータ転送 (Host-Device Data Transfer)](https://github.com/coky-t/owasp-mastg-ja/blob/master/techniques/android/MASTG-TECH-0002.md) を使用して、システムによって撮影されたスクリーンショットをラップトップにコピーし、さらに解析します。システムはスクリーンショットをコンテナ `/data/system_ce/0/snapshots` または `/data/system` に保存します。
