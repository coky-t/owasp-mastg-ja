---
title: Sileo
platform: ios
source: https://github.com/Sileo/Sileo
---

# MASTG-TOOL-0064 Sileo

Sileo は iOS 12 以降でルート済み脱獄とルートなし脱獄の両方で利用できるサードパーティアプリストアです。`apt/dpkg` 用のグラフィカルユーザーインタフェースを提供し、アプリ、バイナリ、Tweak をインストール可能にします。Sileo は Dopamine や palera1n などの人気の脱獄ツールにパッケージ化されています。

> \[!WARNING] 「アプリのインストール」\
> Sileo やその他のサードパーティアプリストア経由でアプリケーションをインストールすると、Apple はそれらを検証しないため、リスクを伴います。不明なアプリケーションをインストールする際には注意してください。

Sileo を使用することで、多くの脱獄ツールをインストールできます。デフォルトでは、Sileo はいくつかのパッケージリポジトリが設定されています。新しいリポジトリを追加するには **Sources** -> **Edit** に移動し、左上の **Add** をクリックします。

この方法でインストールできる便利なツールをいくつか紹介します。

* [Frida (iOS)](MASTG-TOOL-0039.md)
* [SSL Kill Switch 2](MASTG-TOOL-0066.md)
* [GlobalWebInspect](MASTG-TOOL-0137.md)
* [AppSync Unified](MASTG-TOOL-0127.md)

![](../../.gitbook/assets/TOOL-0064-Sileo.png)
