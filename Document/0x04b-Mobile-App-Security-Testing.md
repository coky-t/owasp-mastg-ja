...
- **報告** - クライアントにとって重要なこのフェーズでは、セキュリティテスト担当者は悪用できた脆弱性を報告し、実行可能な危殆化の種類をスコープ (例えば、テスト担当者が不正にアクセスできたデータ) を含めて文書化します 。
...
- **使用時** - アプリケーションがアドレス空間にデータをロードしたとき
- **転送時** - モバイルアプリとエンドポイント間でデータが交換されたときや、デバイス上のプロセスを使用するとき、例、IPC (プロセス間通信) 時など

それぞれの状態に適した調査の度合いはデータの重要度やアクセスの可能性に依存します。例えば、攻撃者はウェブサーバーよりもモバイルデバイスに物理的にアクセスする可能性が高いため、アプリケーションメモリに保持されているデータはコアダンプ経由でアクセスされるウェブサーバー上のデータよりも脆弱になります。
...
- 保護が法的義務であるデータ
- アプリケーション (またはそれに関連するシステム) により生成され、他のデータやシステムを保護するために使用される技術データ (暗号化鍵など)
...
- 内部プロセス、ワークフロー、および組織構造。組織固有の内部プロセスおよびワークフローは [ビジネスロジックを悪用する](https://www.owasp.org/index.php/Testing_for_business_logic "Testing business logic") 機会を生み出す可能性があります。
...
機密性が低いまたは中程度のアプリケーションのデリバリーとデプロイメントは単一のステップにマージされ、デリバリー後に妥当性確認が実行されることがあります。但し、機密性の高いアプリケーションにはこれら二つのアクションを分離して強力な妥当性確認を使用することを強く推奨します。
...
- V1.1: "アプリのすべてのコンポーネントを把握し、それらが必要とされている。"
- V1.3: "モバイルアプリと接続されるすべてのリモートサービスの高次のアーキテクチャが定義され、そのアーキテクチャにセキュリティが対応されている。"
- V1.4: "モバイルアプリのコンテキストで機密とみなされるデータが明確に特定されている。"
- V1.5: "アプリのすべてのコンポーネントは提供する業務上の機能やセキュリティ上の機能の観点で定義されている。"
- V1.6: "モバイルアプリとそれに関連するリモートサービスの脅威モデルが作られ、潜在的な脅威と対策を特定している。"
- V1.7: "すべてのセキュリティコントロールは集約実装されている。"
- V1.10: "セキュリティはソフトウェア開発ライフサイクルのあらゆる部分で対処されている。"
