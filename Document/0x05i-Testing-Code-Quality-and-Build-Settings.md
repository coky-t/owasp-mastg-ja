...
ソースが利用できない場合、アプリを逆コンパイルして JAR ファイルを確認できます。Dexguard や [ProGuard](0x08a-Testing-Tools.md#proguard) が正しく適用されていると、ライブラリに関するバージョン情報が失われていることがよくありますが、そうでなければたいていは特定のライブラリの Java ファイルのコメントにあります。MobSF などのツールはアプリケーションに同梱されている可能性のあるライブラリの解析に役立ちます。ライブラリのバージョンをコメントから、または特定のバージョンで使用されている特定のメソッドから取得できる場合には、手作業でそれらのライセンスを調べることができます。
