...
<img src="../.gitbook/assets/finder_ipad_view.png" alt="" width="100%">

モデル、ストレージ容量、バッテリー情報を含むテキストをクリックすると、代わりにシリアル番号、UDID、モデルを表示します。

<img src="../.gitbook/assets/finder_unveil_udid.png" alt="" width="100%">

UDID を右コピーするとコピーできます。

また、デバイスが USB で接続されていると、macOS のさまざまなコマンドラインツールを介して UDID を取得することも可能です。

*   [I/O Registry Explorer](https://developer.apple.com/library/archive/documentation/DeviceDrivers/Conceptual/IOKitFundamentals/TheRegistry/TheRegistry.html) ツール `ioreg` を使用する:
...
Apple シリコン搭載の Mac では、[互換性のある iPhone および iPad アプリは Mac App Store を通じて配布され、macOS 上で直接実行できます](https://developer.apple.com/documentation/apple-silicon/running-your-ios-apps-in-macos)。この環境は iOS シミュレータとは異なり、別途検出する必要があります。

詳細については [macOS 上で動作する iOS アプリの検出 (iOS Apps Running on macOS Detection)](../knowledge/ios/MASVS-RESILIENCE/MASTG-KNOW-0136.md) を参照してください。

### iOS シミュレータでのテスト

実際の Android デバイスのハードウェアを完全にエミュレートする Android エミュレータとは異なり、iOS シミュレータは iOS デバイスのより高いレベルの _シミュレーション_ を提供します。シミュレータ用のバイナリは、シミュレータを動作する、基盤となる macOS オペレーティングシステムとなる x86 または ARM ベースのアーキテクチャにコンパイルされます。一方、物理デバイス向けにコンパイルされたアプリは ARM ベースの iOS オペレーティングシステム向けにコンパイルされます。つまり、実際のデバイス向けにコンパイルされたアプリはシミュレータでは実行できません。

詳細については [iOS シミュレータ検出 (iOS Simulator Detection)](../knowledge/ios/MASVS-RESILIENCE/MASTG-KNOW-0088.md) を参照してください。

### iOS 仮想デバイスでのテスト

仮想デバイスは実際のデバイス向けにコンパイルされたアプリを実行できます。[Corellium](../tools/generic/MASTG-TOOL-0108.md) は一般に入手可能な唯一の iOS 仮想デバイスです。これは企業向けの SaaS ソリューションで、ユーザーごとのライセンスモデルとなっており、コミュニティライセンスを提供していません。
...
iOS バージョンが異なれば、必要な脱獄テクニックも異なります。[お使いの iOS バージョンで利用可能な脱獄が公開されているかどうかを確認してください](https://ios.cfw.guide/) 。偽ツールやスパイウェアに注意してください。脱獄グループや製作者の名前に似たドメイン名の背後に隠れていることがよくあります。

iOS の脱獄シーンは急速に進化しているため、最新のインストラクションを提供することは困難です。しかし、現在信頼できる情報源をいくつか紹介できます。

* [The Apple Wiki](https://www.theapplewiki.com/)
* [Reddit Jailbreak](https://www.reddit.com/r/jailbreak/)
* [AppleDB](https://appledb.dev/)

> \[!WARNING] デバイスに行ういかなる改変も、ご自身の責任で行ってください。脱獄は一般的に安全ですが、物事はうまくいかないことがあり、デバイスが文鎮化してしまうかもしれません。いかなる損害に対しても、あなた自身以外の第三者が責任を負うことはありません。
