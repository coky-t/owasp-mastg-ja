...
セキュリティテスト入門の章の [iOS シミュレータ上でのテスト](0x06b-Basic-Security-Testing.md "Testing on the iOS Simulator") セクションで説明したように、利用可能なシミュレータは Xcode に同梱されているものだけです。シミュレータバイナリは ARM コードではなく x86 コードにコンパイルされており、実デバイス (ARM アーキテクチャ) 用にコンパイルされたアプリはシミュレータでは動作しないため、幅広い _エミュレーション_ 選択肢が利用できる Android とは対照的に、 iOS アプリに関して _シミュレーション_ 保護はそれほど気にする必要はありませんでした。
