...
多くの場合、いくつかの秘密の機能を難読化することは焦点がずれていると考えられます。実際の目的として、攻撃者は難読化された機能に関するすべての詳細を知る必要はありません。例えば、難読化されたプログラムの機能が入力値を受け取り、それを使用して、わかりにくい方法で出力値を計算することができます (例えば、隠し鍵を使用した暗号捜査を通じて) 。大部分のシナリオでは、攻撃者の目標はプログラムの機能性を複製すること、すなわち、攻撃者が所有するシステム上で同じ出力値を計算することです。コードを丹念にリバースエンジニアリングするのではなく、単純に実装全体をコピーおよび再利用するのはなぜでしょう。攻撃者がブラックボックスの中を見る必要がある理由はありますか。

この種の攻撃は code lifting として知られており、一般に DRM やホワイトボックス暗号実装を破るために使用されます <sup>[18]</sup> 。例えば、デジタルメディアの使用をバイパスしようとする攻撃者は、プレーヤーから暗号化ルーチンを抽出し、偽造プレーヤーにそれを含めるだけで、含まれている使用ポリシーを強制することなくデジタルメディアを復号できます <sup>19</sup> 。ホワイトボックス実装の設計者は、鍵を実際に抽出することなく暗号ルーチンを復号ルーチンに変換できる <sup>[20]</sup>、という別の問題に対処する必要があります。

保護されたアプリケーションには code lifting 対策を組み込む必要があります。実際には、バイナリが実行される特定の環境 (ハードウェア、デバイス、クライアント/サーバーインフラストラクチャ) に難読化された機能をバインドすることを意味します。できれば、保護された機能は特定の正当なコンピューティング環境においてのみ正しく実行すべきです。例えば、難読化された暗号アルゴリズムは環境から収集されたデータを使用してその鍵 (または鍵の一部) を生成します。アプリの機能を特定のハードウェアに結び付ける技法はデバイスバインディングと呼ばれています。

それでも、(ブラックボックスを完全にリバースエンジニアリングするのではなく) アプリとその環境のやり取りを監視することは比較的簡単です。実際には、デバイスの IMEI や MAC アドレスなどの単純なハードウェアプロパティがデバイスバインディングを達成するためによく使用されます。これらの環境プロパティを詐称するために必要な労力は、難読化された機能を完全に理解するために必要とされる労力よりもはるかに少なくなります。

これが意味するところは、ほとんどの実用的な目的として、難読化されたアプリケーションのセキュリティはそれを実装しているデバイスバインディングと同程度でしかないということです。デバイスバインディングを有効にするには、システムまたはデバイスの特定の性質をさまざまな難読かレイヤーと深く組み合わせる必要があり、これらの性質を秘密の方法で (理想的には、メモリから直接コンテンツを読み取ることによって) 決定する必要があります。高度なデバイスバインディング手法は DRM やマルウェアに展開されることが多く、この分野ではいくつかの研究が公開されています <sup>[22]</sup> 。
