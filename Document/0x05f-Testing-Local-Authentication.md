...
- ロック画面で保護される鍵を作成します (ユーザーが直近 30 秒以内に自分のデバイスをアンロックしていることを確認するか、再度アンロックする必要があります) 。この時間が長すぎると、デバイスをアンロックしたユーザーとアプリを使用しているユーザーが同じであることを確認することが難しくなることに注意します。
...
鍵がアプリケーションの実動作に必要であることを確認します。鍵を生成するだけで、ローカルストレージのアクセス、アプリの機能の使用、リモートエンドポイントのアクセスが必要ない場合には、資格確認フローをバイパスする可能性があります。
