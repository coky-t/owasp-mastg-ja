...
ソーシャルメディアアカウントと連携する必要がある **ネイティブアプリ** に OAuth2 を実装する際、WebView や WKWebView といった埋め込み型ユーザーエージェントの使用は常に避ける必要があります。これは、ホストアプリが埋め込みビューを完全に制御できるため、ユーザー名やパスワードなどのユーザー入力を記録できてしまうからです。

代わりに、より安全な実装は、システムブラウザやアプリ内ブラウザタブなどの _外部ユーザーエージェント_ を使用するものです。これらは別のインスタンスで動作するため、ホストアプリがユーザーのクレデンシャルを捕捉することを防ぎます。

RFC 8252 ではプライベートユースの URI スキームの使用を許可していますが、可能な限り避けるべきです。別のアプリが同じカスタムスキームを登録し、認可コードを傍受する可能性があるためです。

代わりに、Android App Links や iOS Universal Links といった、Claimed "https" Scheme URI を可能な限り使用すべきです。それらは、オペレーティングシステムがリダイレクト URI を正当なアプリに関連付けることを可能になるためです。
