...
## ハードコードされた暗号鍵による対称暗号化のテスト (MSTG-CRYPTO-1)
...
## 暗号化手段の再使用 (MSTG-CRYPTO-5)

暗号化手段は互いに他と混在していないことを確認します。例えば、公開鍵で署名したり、署名に使用された鍵ペアを使用して暗号化することはしていません。

## 脆弱な乱数生成器 (MSTG-CRYPTO-6)
