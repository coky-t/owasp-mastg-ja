...
デバッグについて語る場合、一般的には対話的デバッグセッションを意味し、デバッガは実行中のプロセスにアタッチされます。対照的に、*トレース* は API コールなどのアプリの実行に関する情報の受動的なログ出力を指します。これは、デバッグ API、関数フック、カーネルトレース機能を含む多くの方法で実行できます。改めて、これらの技法の多くは OS ごとの「リバースエンジニアリングと改竄」の章で説明します。
