...
| ネイティブライブラリのサポート | 通常、ネイティブライブラリは ARM デバイス用にビルドされているため、物理デバイス上で動作します。 | x86 CPU で動作するため、一部のエミュレータはパッケージ化されたネイティブライブラリを実行できないかもしれません (利用可能なワークアラウンドについては以下の [x86 エミュレータ上での ARM バイナリ変換](#arm-binary-translation-on-x86-emulators) を参照してください)。 |
...
##### x86 エミュレータ上での ARM バイナリ変換

> [!WARNING]
> バイナリ変換を通じて x86 上で ARM コードを実行すると、アプリの動作が変化したり、クラッシュを発生する可能性があります。OWASP MASTG に貢献する際は、バイナリ変換を使用したかどうかを明記し、そのエミュレータ、システムイメージ、変換の実装を特定します。変換はセキュリティテストの結果に影響を及ぼす可能性があるため、可能な限り ARM デバイス上で関連するテストやデモを確認してください。
> ほとんどの Android デバイスは ARM ベースのプロセッサを使用しており、多くの Android アプリケーションは ARM アーキテクチャ (`armeabi-v7a` や `arm64-v8a`) 向けにコンパイルされたネイティブライブラリを含んでいます。こうしたアプリケーションを x86 または x86_64 エミュレータでテストする場合、エミュレータは ARM ネイティブコードを実行するメカニズムを提供する必要があります。

**公式の Android エミュレータ (AVD):** Android 11 (API レベル 30) 以降、Google API および Google Play システムイメージにはビルトインの ARM から x86 への変換を含んでいます。サポートのレベルはシステムイメージによって異なります。x86 イメージは x86 と ARMv7 (`armeabi-v7a`) の両方の ABI をサポートし、x86_64 イメージはさらに ARM64 (`arm64-v8a`) をサポートします。Android OS と Android Runtime (ART) は x86 上でネイティブに動作しますが、アプリケーションのプロセスによって必要とされる ARM バイナリはそのプロセス内で x86 に変換されます。これは、フルシステム ARM エミュレーションのパフォーマンスオーバーヘッドを回避します。詳細については、[Android エミュレータでの ARM アプリの実行に関する Google の発表](https://android-developers.googleblog.com/2020/03/run-arm-apps-on-android-emulator.html) を参照してください。

**カスタム VM とネイティブブリッジ:** Genymotion のような x86 ベースのカスタム環境は一般的に標準では ARM 変換を提供していません。Android は、バイナリ変換実行をロードするために使用できるネイティブブリッジメカニズムを提供しています。ネイティブブリッジ実装を設定するには `ro.dalvik.vm.native.bridge` システムプロパティが使用されます。

Android x86 環境でみられる一般的な変換実装には以下があります。

- `libhoudini`: 一部の Android x86 環境で使用されている、ARM から x86 への独自の変換実装。
- `libndk_translation`: 互換性のある Android エミュレータのシステムイメージに含まれている、ARM から x86 への変換実装。

コミュニティが保守するスクリプトやフラッシュ可能なアーカイブを使用して、一部のカスタム VM に ARM 変換サポートを追加できます。しかしながら、これらのパッケージはバージョン固有であり、異なる Android システムイメージ間では動作しない可能性があります。

セキュリティテスト時に ARM バイナリ変換を使用する際は、以下の制限事項やセキュリティ上の側面を考慮してください。

- **動的計装**: [Frida](../tools/generic/MASTG-TOOL-0031.md) などのツールは、制限に遭遇したり、変換レイヤを通じて実行するネイティブ ARM 関数のフックに失敗する可能性があります。
- **アプリケーションの互換性**: アーキテクチャ固有の動作に依存するアプリケーションや、環境チェックを実行するアプリケーションは、変換下に動作が異なったり、失敗する可能性があります。そのため、変換レイヤを通じて正常に実行できたとしても、物理的な ARM デバイス上で同じ動作を保証しません。
- **サプライチェーンのリスク**: サードパーティリポジトリから入手したコミュニティ変換パッケージは、root 権限を必要としたり、システムパーティションやシステムライブラリを改変する必要がある可能性があります。そのようなパッケージは信頼できるソースからのみインストールし、隔離されたテスト環境で使用します。
