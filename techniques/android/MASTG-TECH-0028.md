...
- `tx_queue` と `rx_queue`: カーネルメモリ使用の観点での送出データキューと受入データキュー。これらのフィールドは接続がどの程度アクティブに使用されているかの示唆を与えます。
...
- `Recv-Q` と `Send-Q`: 受信キューと送信キューに関する統計。接続がどの程度アクティブに使用されているかを示します。
