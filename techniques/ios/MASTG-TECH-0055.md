...
# MASTG-TECH-0055 再パッケージ化したアプリをデバッグモードで起動する (Launching a Repackaged App in Debug Mode)

アプリをデバイスにインストールした後、デバッグモードで起動する必要があります。SpringBoard 経由でアプリを起動する場合はそうではありません (アプリケーションがクラッシュします) が、[アプリのインストール (Installing Apps)](https://github.com/coky-t/owasp-mastg-ja/blob/master/techniques/ios/MASTG-TECH-0056.md) で説明されているように、さまざまなツールを使用することで可能です。アプリケーションがデバッグモードで実行されている場合、Frida を `Gadget` という名前でプロセスに注入できます。
