...
この場合、ファイルは `@xml` (/res/xml と同じ) にあり、名前は "network\_security\_config" (異なることがあります) です。 "res/xml/network\_security\_config.xml" として見つけることができるはずです。コンフィグレーションが存在する場合、システムログ ([システムログの監視 (Monitoring System Logs)](https://github.com/coky-t/owasp-mastg-ja/blob/master/techniques/android/MASTG-TECH-0009.md)) に以下のイベントが表示されるはずです。
